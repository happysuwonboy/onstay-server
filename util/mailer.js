...
const resetPwHtml = (token) => `
    ...
    <h3>안녕하세요, onstayhouse입니다! 아래 링크를 클릭하여 비밀번호를 재설정해주세요.</h3>
     <a href='${JSON.parse(process.env.FRONTEND_ORIGINS)[0]}/find/pw/reset?token=${token}'>
        비밀번호 재설정
     </a>
    </div>
`
...
const postAnswerNotiHtml = (user_id, question_title) => `
    <div>
     ...
     <a href='${JSON.parse(process.env.FRONTEND_ORIGINS)[0]}/mypage?showContent=MyQNA&subContent=MyQNAList'>문의 내역 보러가기</a>
    </div>
`
